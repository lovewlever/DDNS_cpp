...
{
    std::cout << "Worker Type: " << ipvConf.Type << std::endl;
    ...
    {
        std::cerr << "Invalid Type, Type must be IPV4 or IPV6" << std::endl;
    }
}

void DDNSWorker::distributionProvider(const int32_t ipvInt, const YamlConfig::IpvConfig &ipv) const
{
    std::cout << "Worker Provider: " << ipv.Provider << std::endl;
    ...
    } else if (ipv.Provider == YamlConfig::IpvConfProviderMachine)
    {
        if (ipvInt == 4)
        {
            std::cerr << ipv.Provider << " only support windows ipv6" << std::endl;
        } else
        {
    ...
    {
        std::cerr << "Invalid Provider, Provider must be: SSH_OPENWRT or NETWORK or MACHINE" << std::endl;
    }
}

void DDNSWorker::execIpv4Network(const YamlConfig::IpvConfig &ipvConf) const
{
    std::cout << "Worker Ipv4Network: ..." << std::endl;
    if (ipvConf.NetworkConfig.empty())
    {
        std::cerr << "No IPv4 network configuration" << std::endl;
        return;
    }
    std::string ipGet{};
    ...
    {
        std::cerr << "No ip was obtained" << std::endl;
    }
}

void DDNSWorker::execIpv4SSHOpenWRT(const YamlConfig::IpvConfig &ipvConf) const
{
    std::cout << "Worker Ipv4SSHOpenWRT: ..." << std::endl;
    ...
    {
        std::cerr << "No IPv4 SSHOpenWRT configuration" << std::endl;
        return;
    }
    ...
    {
        std::cerr << "Failed to get IPv4 SSHOpenWRT: " << msg << std::endl;
    }
...
{
    std::cout << "Worker Ipv6Network: ..." << std::endl;
    if (ipvConf.NetworkConfig.empty())
    {
        std::cerr << "No IPv6 network configuration" << std::endl;
        return;
    }
    std::string ipGet{};
    for (const auto &ipv: ipvConf.NetworkConfig)
    ...
    if (!ipGet.empty())
    {
        if (ipvConf.Cloud == YamlConfig::IpvConfCloudAliCloud)
        {
            ...
            const AliCloudReport ar{};
            ar.addOrUpdateDomain(AccessKeyId, AccessKeySecret, ipvConf.Subdomain,
                                 ipvConf.Domain, ipGet, "AAAA", ipvConf.TTL);
        } else
        {
            std::cerr << "Currently only supports Ali Cloud: [ALICLOUD]" << std::endl;
        }
...
{
    std::cout << "Worker Ipv6Machine: ..." << std::endl;
